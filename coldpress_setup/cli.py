...
def user(config_file, output_dir):
    ...
    try:
        validate_user_config(config_data)
        # Convert back to dict for compatibility with downstream code
        config = config_data
    except ValidationError as e:
...
def generate_cluster_config(config_file, output_dir):
    ...
    if nodes:
        ...
        with open(label_script_path, "w") as f:
            f.write("#!/bin/bash\n")
            f.write(
                f"# Generated by coldpress-setup on {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')}\n"
            )
            ...
            for node_id, node in enumerate(nodes):
                ...
                f.write(
                    f"oc label node {node['hostname']} coldpress.node={node_id} --overwrite\n"
                )
            f.write("\necho ''\n")
